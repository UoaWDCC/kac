...
export default function Modal({
  ...
  useEffect(() => {
    ...
    setEthnicity(exec.ethnicity ?? "");
    setDegree(exec.degree ?? "");
    setMBTI(exec.mbti ?? "");
    setFact(exec.fact ?? "");
    setSponsor(exec.sponsor ?? "");
    setGreenFlag(exec.greenFlag ?? "");
    setRedFlag(exec.redFlag ?? "");
    setEmojis(exec.emojis ?? "");
  }, [exec]);
      ...
      <div className="font-alan-sans text-sm bg-white rounded-2xl pr-6 pt-8 justify-center relative border-box min-w-[48vw] h-fit max-h-[90vh] flex flex-col gap-6">
        <div className="overflow-y-auto pl-12 pr-6">
          ...
          <form className="flex flex-col gap-8">
            ...
            <div className="flex flex-col gap-1 w-full">
              <h3 className="text-lg font-bold">Favourite KAC Sponsor</h3>
              <input
                id="sponsor"
                type="text"
                value={sponsor}
